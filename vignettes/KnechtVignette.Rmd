...
## Vignette Info and Introduction
  This vignette uses the `Knecht` data from Knecht (2006) and Knecht (2008) to provide an illustrative example of estimating an ego-Temporal Exponential Random Graph Model (ego-TERGM) using the `egoTERGM` package [@knecht2006networks, @knecht2008friendship].  It begins by estimating an ego-TERGM on the Knecht friendship network, a network of 26 students in a Dutch classroom measured in four time-periods.  In addition to data on the friendships between students there is vertex data on a variety of demographic and behavioral factors.  Fur a detailed explanation of the data, I refer the reader to the original publications on the data or to the `xergm.common` help page for the `knecht` data: `??knecht`.  

Much of the code for estimating the ego-TERGM is a modification of code originally produced for its cross-sectional equivalent [@salter2015role].  As such, I am exceptionally thankful to Michael Salter-Townshend and Brendan Murphy for making their code publically accessible so that I may modify it for the estimation of the ego-TERGM.  Rest assured, however, that any errors you may encounter in this package are solely mine.  

This vignette will proceed step-by-step in presenting the typical workflow for estimating and interpreting an ego-TERGM using `egoTERGM`.  
...
## Importing Knecht Network
  ...
  The following chunk of code imports the Knecht friendship network gathered by Andrea Knecht and provided in the `xergm.common` package [@leifeld2014xergm].  It also uses some code from the `xergm.common` package to prepare the network for the `ego_tergm` function, note that it produces a list of network objects.
...
Once the data is imported and transformed into a `list` of `network` objects with relevant attributes appended to each network in that list, we can proceed with estimating the ego-TERGM using `ego_tergm`.  Underneath the hood, quite a bit is occuring when the user uses this function.  First, the function transforms the list of longitudinally observed networks into a list of longitudinally observed egonetworks.  From that point, the initialization proceedure described by Campbell (2017) is used, including the estimation of a TERGM via bootstrapped pseudolikelihood and subsequent clustering of TERGM parameters using k-means clustering [@campbell2017inferring].  Upon initialization, an Expectation Maximization (EM) algorithm is used to find MLE centroids and role assignments.  
...
*`min_size`: The minimum size that an egonetwork at a particular time priod must achieve to be included.
*`roles`: The number of roles that one expects.
*`add_drop`: Whether nodes drop in or drop out of the network.
*`directed`: If the network is directed or not.
*`edge_covariates`: In the `form` call, are edge_covariates required?
*`seed`: Seed set for replication.
*`R`: The number of bootstrap replications for the TERGM to be used to generate initial values.
*`parallel`: Should parallel computing be used? If no, specify "no", otherwise specify "snow" or "multicore".
